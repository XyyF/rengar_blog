...
(0, _util.addLoadEvent)(function () {
	new _share2.default().init();
	new _viewer2.default().init();
	new _gotop2.default().init();
	new _gitment2.default().init();
...
var Share = function () {
    ...
    (0, _createClass3.default)(Share, [{
        ...
        value: function init() {
            ...
            (0, _event.on)(document.querySelector('.mask'), 'click', Share.hideWX);
            (0, _event.on)(document.querySelector('.js-modal-close'), 'click', Share.hideWX);
        }
    }], [{
        key: 'generate',
        value: function generate(url, opts) {
            var urls = url.replace(/<%-sUrl%>/g, encodeURIComponent(opts.sUrl)).replace(/<%-sTitle%>/g, opts.sTitle).replace(/<%-sDesc%>/g, opts.sDesc).replace(/<%-sPic%>/g, encodeURIComponent(opts.sPic));

            window.open(urls);
        }
    }, {
        key: 'handleClick',
        value: function handleClick(type, opts) {
            if (type === 'weibo') {
...
var Gotop = function () {
    ...
    (0, _createClass3.default)(Gotop, [{
        ...
        key: 'backTop',
        value: function backTop(domE, ctn, distance) {
            if (!domE) return;
            var timer = null;
            ...
            domE.onclick = function () {
                ...
                timer = setInterval(function () {
                    ...
                    var speed = ct / 7;
                    var diff = speed < 1 ? Math.ceil(speed) : Math.floor(speed);
                    ct -= diff;
                    if (ct > 0) {
                        //如果与顶部的距离大于零
...
var Gitment = function () {
    ...
    (0, _createClass3.default)(Gitment, [{
        key: 'init',
        value: function init() {
            var gitment = new _rengarGitment2.default({
                owner: 'XyyF',
                repo: 'XyyF.github.io',
                oauth: {
                    client_id: '49eac8228181b2222d57',
                    client_secret: '96ecc2513c58eebbb14e304c344a322da5b73dee'
                }
            });
...
var RengarRipple = function () {
    function RengarRipple(element) {
        (0, _classCallCheck3.default)(this, RengarRipple);
    ...
    (0, _createClass3.default)(RengarRipple, [{
        key: 'downHandler',
        value: function downHandler(index, event) {
            ...
            if (!this.rippleElement.style.width && !this.rippleElement.style.height) {
                var rect = this.targetElement[index].getBoundingClientRect();
        ...
        value: function init() {
            ...
            Array.prototype.forEach.call(this.targetElement, function (el, index) {
                /**
                 * conversion UI:
                ...
                rippleContainer.classList.add(_this.CssClasses.RIPPLE_CONTAINER);
                var rippleElement = document.createElement('span');
                rippleElement.classList.add(_this.CssClasses.RIPPLE_ELEMENT);
                _this.rippleElementList.push(rippleElement);
                rippleContainer.appendChild(rippleElement);
                el.appendChild(rippleContainer);

                // binding downHandler and upHandler
                _this.boundDownHandler = _this.downHandler.bind(_this, index);
                _this.boundUpHandler = _this.upHandler.bind(_this);
                el.addEventListener('mousedown', _this.boundDownHandler);
                el.addEventListener('touchstart', _this.boundDownHandler);
                el.addEventListener('mouseup', _this.boundUpHandler);
                el.addEventListener('mouseleave', _this.boundUpHandler);
                el.addEventListener('touchend', _this.boundUpHandler);
                el.addEventListener('blur', _this.boundUpHandler);
            });
        }
    }]);
    return RengarRipple;
